...
import {getMusicByDecade, getTrackByName} from "../util/Client.ts";
        ...
        useEffect(() => {
                getMusicByDecade(selectedDecade).then(
                    (tracks: TrackObject[]) => {
                        setIsLoading(true);
                        setTracks(tracks);
                        const art = tracks ? tracks[currentTrack].imagePath : '';
                        if (art.length < 1) {
        ...
        return (
            <>
                ...
                <Card sx={{
                    ...
                    {tracks && tracks.length > 0 && <Gemini song={tracks[currentTrack].name}/>}
                </Card>
